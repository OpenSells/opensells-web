        ...
        <div className="grid gap-6 sm:grid-cols-3">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.key}
                className={`relative rounded-2xl p-6 flex flex-col ${isPopular ? 'bg-violet-500 text-white shadow-xl ring-2 ring-violet-500' : 'bg-white border border-slate-200 shadow-sm'}`}
              >
                {isPopular && (
                ...
                <div className="mb-6">
                  ...
                  <p className={`text-xs mb-4 ${isPopular ? 'text-violet-100' : 'text-slate-400'}`}>{plan.description}</p>

                  {isFree ? (
                    <div className="flex items-end gap-1">
                      <span className={`text-4xl font-extrabold ${isPopular ? 'text-white' : 'text-slate-900'}`}>€0</span>
                      <span className={`text-sm mb-1 ${isPopular ? 'text-violet-100' : 'text-slate-400'}`}>{t('per_month')}</span>
                    </div>
                  ) : (
                    <div>
                      <div className="flex items-baseline gap-2 mb-1">
                        ...
                        <span className={`text-sm line-through ${isPopular ? 'text-violet-200' : 'text-slate-400'}`}>
                          €{originalPrice}
                        </span>
                      </div>
                      <p className={`text-xs font-semibold ${isPopular ? 'text-violet-100' : 'text-violet-600'}`}>
                ...
                <ul className="space-y-2.5 flex-1 mb-8">
                  {plan.features.map((f, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm">
                      <svg className={`h-4 w-4 flex-shrink-0 mt-0.5 ${isPopular ? 'text-white' : 'text-violet-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                      </svg>
                      <span className={isPopular ? 'text-violet-50' : 'text-slate-600'}>{f}</span>
                    </li>
                  ))}
                </ul>
