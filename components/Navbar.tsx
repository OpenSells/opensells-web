      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600">
            <Link href={`#features`} className="hover:text-slate-900 transition">{t('features')}</Link>
            <Link href={`#pricing`} className="hover:text-slate-900 transition">{t('pricing')}</Link>
            <Link href={`#blog`} className="hover:text-slate-900 transition">{t('blog')}</Link>
          </nav>

          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push(switchPath)}
              className="hidden sm:flex h-8 items-center rounded-md border border-slate-200 px-2.5 text-xs font-semibold text-slate-500 hover:bg-slate-50 transition uppercase"
            >
              {otherLocale}
            </button>
            <Link
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-slate-100 py-3 space-y-1">
            ...
            <div className="pt-2 flex items-center gap-3 px-2">
              <button
                onClick={() => router.push(switchPath)}
                className="text-xs font-semibold text-slate-500 uppercase border border-slate-200 rounded-md px-2 py-1"
              >
                {otherLocale}
              </button>
